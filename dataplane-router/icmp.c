...
void respond_to_icmp(char *packet, size_t len, int interface)
{
	...
	// create ipv4 header
	int pack_data_len = len - sizeof(struct ether_header) -
...
void send_dest_unreachable_icmp(char *packet, size_t len, int interface)
{
	...
	// create ipv4 header
	int pack_iphdr_and_data_len =
		MIN(len - sizeof(struct ether_header), sizeof(struct iphdr) + 64);

	rp_ip_hdr->ihl = 5;
	rp_ip_hdr->version = 4;
	rp_ip_hdr->tos = 0;
	rp_ip_hdr->tot_len = htons(sizeof(struct iphdr) + sizeof(struct icmphdr) +
							   pack_iphdr_and_data_len);
	rp_ip_hdr->id = htons(1);
	rp_ip_hdr->frag_off = htons(0);
	rp_ip_hdr->ttl = 64;
	rp_ip_hdr->protocol = ICMP;
	...
	memcpy(response_packet + sizeof(struct ether_header) +
			   sizeof(struct iphdr) + sizeof(struct icmphdr),
		   pack_ip_hdr, pack_iphdr_and_data_len);

	rp_icmp_hdr->checksum = 0;
	rp_icmp_hdr->checksum = htons(checksum(
		(void *)rp_icmp_hdr, sizeof(struct icmphdr) + pack_iphdr_and_data_len));

	int response_packet_len = sizeof(struct ether_header) +
		sizeof(struct iphdr) + sizeof(struct icmphdr) + pack_iphdr_and_data_len;
