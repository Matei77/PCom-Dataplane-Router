...
int main(int argc, char *argv[])
{
	...
	hashtable_t *arp_cache = ht_create(INITIAL_BUCKETS_NR, hash_function_int,
									   compare_function_uint32);

	/* create list of packet that are waiting for arp reply */
	linked_list_t *arp_waiting_list =
		ll_create(sizeof(struct waiting_packet_t));
	...
	while (1) {
		...
		/* checks if the received package was sent to the correct MAC
		 * destination */
		...
		if (ntohs(eth_hdr->ether_type) == ETHERTYPE_IP) {
			process_ip_packet(packet, len, interface, rt_trie_root, arp_cache,
							  arp_waiting_list);
		}

		/* process arp packet */
		if (ntohs(eth_hdr->ether_type) == ETHERTYPE_ARP) {
			process_arp_packet(packet, len, interface, arp_cache,
							   arp_waiting_list);
		}
	}
}